...
public class Args {
    ...
    private static final DBSizeMonitor<DBMonitorProperty, DBSize> dbMonitor = new DefaultDBSizeMonitor();

    private static final TableStructureMonitor<DBMonitorProperty, TableStructure> tbMonitor = new DefaultTableStructureMonitor();
    ...
    @Parameter(names = "-t", description = "数据库类型 0-postgresql 1-mysql 2-oracle 3-sqlserver")
    public int type = 0;
    ...
    public static Database toArgsObject(String... args) {
        ...
        property.setDatabase(argsObj.db);
        property.setHost(argsObj.host);
        property.setPort(argsObj.port);
        ...
        DBSize dbSize = dbMonitor.queryDBSize(property);
        ...
        for (int i = 0; i < detailSize.size(); i++) {
            ...
            TableStructure tableStructure = tbMonitor.queryTableStructure(property, tableSize.getTableName());
